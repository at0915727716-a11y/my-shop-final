...
if "%choice%"=="7" start http://localhost:3000/shop.html
if "%choice%"=="8" start http://localhost:3000/login.html
if "%choice%"=="9" (
    echo 🔧 إصلاح المشاكل...
    npm install
    pm2 delete absi-stor
    pm2 start ecosystem.config.js
    pm2 save
    echo ✅ تم الإصلاح
    timeout /t 3
